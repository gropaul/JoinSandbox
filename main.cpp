...
const string BUILD_QUERY = "SELECT key, FROM build_100m LIMIT 4_000;";
// // // // // const string BUILD_QUERY = "SELECT CAST(range as uint64) as key FROM range(64);";
const string PROBE_QUERY = "PRAGMA disabled_optimizers='top_n';WITH values AS (SELECT key FROM probe_100m LIMIT 80_000_000) SELECT * FROM values ORDER BY hash(key*23);";
...
// const string BUILD_QUERY = "SELECT CAST(range as uint64) as key FROM range(128)";
// const string PROBE_QUERY = "SELECT CAST(range as uint64) as key FROM range(256)";
